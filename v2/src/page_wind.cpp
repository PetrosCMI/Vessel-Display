...
static void build_rose(lv_obj_t* parent, int x, int y, int sz) {
    ...
    lv_label_set_text(wind_awa_lbl, "AWA ---°");
    ...
    lv_label_set_text(wind_twa_lbl, "TWA ---°");
...
static void build(lv_obj_t* tab) {
    ...
    wind_twd = make_instr_card(tab, "TWD", "°T", false);
...
static void update() {
    ...
    // AWS — alarm-aware colouring
    bool wind_alarm = alarm_is_active(ALARM_WIND_SPEED_MAX);
    const char* su = fmt_speed(d.aws_kts / 1.94384f, buf, sizeof(buf));
    instr_card_set(wind_aws, buf, su, false, wind_alarm);

    su = fmt_speed(d.tws_kts / 1.94384f, buf, sizeof(buf));
    instr_card_set(wind_tws, buf, su);

    fmt_angle(d.twd_deg, buf, sizeof(buf));
    instr_card_set(wind_twd, buf, "°T");

    if (!isnan(d.awa_deg)) {
        lv_arc_set_value(wind_arc, (int)d.awa_deg);
        snprintf(buf, sizeof(buf), "AWA %+.0f°", d.awa_deg);
        lv_label_set_text(wind_awa_lbl, buf);
    }
    if (!isnan(d.twa_deg)) {
        snprintf(buf, sizeof(buf), "TWA %+.0f°", d.twa_deg);
        lv_label_set_text(wind_twa_lbl, buf);
    }
}

static struct WindPageRegistrar {
    WindPageRegistrar() {
        ui_register_page({ LV_SYMBOL_UP " WIND", build, update });
    }
} _wind_reg;

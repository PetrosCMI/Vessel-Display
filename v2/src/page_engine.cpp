...
static void build(lv_obj_t* tab) {
    ...
    eng_coolant = make_instr_card(tab, "COOLANT", "°C", false);
...
static void update() {
    ...
    fmt_volts(d.alt_v, buf, sizeof(buf));
    instr_card_set(eng_alt, buf, "V",
        d.alt_v < 12.2f || d.alt_v > 14.8f,
        d.alt_v < 11.8f);
}

static struct EnginePageRegistrar {
    EnginePageRegistrar() {
        ui_register_page({ LV_SYMBOL_SETTINGS " ENG", build, update });
    }
} _engine_reg;
